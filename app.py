...
def sync_logic():
    """ Enforces: If 'None' selected, clear others. If others selected, clear 'None'. """
...
def reset_demo():
    """ Fully restores the app to the original blank state. """
    st.session_state.view_mode = 'original'
    st.session_state.challenges_selection = ["None"]
    # Resetting keys clears the dropdowns to index 0 (blank)
...
if st.session_state.view_mode == 'original':
    ...
    with col2:
        ...
        st.write("A versatile layer for seasonal transitions. Features a smooth full-length zipper and a tailored fit.")
        ...
        with st.expander("FitNexus Intelligence (Check My Fit)", expanded=True):
            ...
            if st.button("Run Analysis"):
                if not real_issues:
                    st.success("Analysis complete: High-confidence match for your profile.")
                else:
